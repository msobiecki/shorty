...
describe("Search Short Action", () => {
  ...
  test("create short success on found", () => {
    fetchMock.getOnce('http://localhost:8000/api/shorty/TEST_SLUG', {
      status: 200,
      body: { status: "success" },
  ...
  test("create short error on not found", () => {
    ...
    const expectedActions = [
      { type: SEARCH_SHORT_REQUEST, payload: { status: "pending" }  },
      { type: THROW_NOTIFICATION, payload: { status: "error", message: "NOTIFICATION_SEARCH_SHORT_NOT_FOUND" }  },
  ...
  test("create short error", () => {
    ...
    const expectedActions = [
      { type: SEARCH_SHORT_REQUEST, payload: { status: "pending" }  },
       { type: THROW_NOTIFICATION, payload: { status: "error", message: "NOTIFICATION_SEARCH_SHORT_REQUEST_ERROR" } },
      { type: SEARCH_SHORT_ERROR, payload: { status: "error" } }
    ]
    const store = mockStore({ shorts: [] })
